...
const Profile = ({user}) => {
  ...
  useEffect(() => {
    setFormData({ name: user.data.name, email: user.data.email,photo: user.data.photo,gender: user.data.gender,bloodType: user.data.bloodType });
  },[user]);
  ...
  const submitHandler = async (event) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/user/${user.data._id}`, {
        method: "PUT",
        headers: {
    ...
    <form onSubmit={submitHandler}>
      ...
      <div className="mt-7">
        <button
          ...
          type="submit"
          className="w-full bg-primaryColor text-white text-[18px] leading-[30px] rounded-lg px-4 py-3 min-h-[50px]" // Add min height for loader space
        >
          {loading ? (
